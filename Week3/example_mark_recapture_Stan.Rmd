...
\newpage
## Do the analysis using Monte Carlo
...
We can now use the vector Nsamp as a collection of random samples from the posterior distribution. Hence let's calculate the mean and 10 % and 90 % quantiles
...
model{
  ...
  R ~ binomial(C, M / N);          // observation model (likelihood)
}
"
```
...
# target  = log ( p(N|R,M,C) ) \propto + log( Bin(R|C,M/N) ) + log( p(N) )  
# 
# target += binomial_lpmf(R|C, M / N)
#   <=>
# target = target + binomial_lpmf(R|C, M / N)
#   
# binomial_lpmf = log( Bin(R|C,M/N) )
